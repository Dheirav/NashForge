# Replay: results/cfr/ladder169l_v6 on the logged hands

2559 decisions from 1107 hands (matches labelled v5*); baseline results/cfr/ladder169l.

## Coverage

logged misses 141, baseline lookup misses 143 (these should match), new-set misses 126; 2409 decisions compared.
...
|---|---|---|---|---|
| preflop | 985 | 0.57 | 0.45 | 455 (46%) |
| flop | 628 | 0.63 | 0.61 | 113 (18%) |
| turn | 468 | 0.64 | 0.59 | 108 (23%) |
| river | 328 | 0.65 | 0.60 | 80 (24%) |

Most common changes of most-likely action (baseline → new):

- raise ½ → check/call: 197
- check/call → raise ½: 131
- raise pot → raise ½: 75
- raise pot → check/call: 72
- check/call → fold: 48
- raise ½ → raise pot: 46
- check/call → raise pot: 40
- raise 2× → check/call: 28
...
Mean call probability over all 12: baseline 0.76, new 0.73.
...
**-9,638 vs mr_hide**, hand 75 (97131196), we held Qc 8c, they showed 2h As.
  - preflop board -              key `` to call 150 → played **raise pot**; new set: check/call 0.30, raise ½ 0.62, raise pot 0.07, raise 2× 0.01
  - flop    board 4c Ah 4d       key `31/2` to call 990 → played **check/call**; new set: check/call 1.00
  - turn    board 4c Ah 4d Ad    key `31/21/2` to call 2,494 → played **check/call**; new set: fold 0.23, check/call 0.77
  - river   board 4c Ah 4d Ad Qs key `31/21/21/5` to call 5,254 → played **check/call**; new set: fold 0.32, check/call 0.68
...
**-8,346 vs r0ckGarden**, hand 55 (524b6f3a), we held Qc 3c, they showed 8d 8s.
  - preflop board -              key `` to call 100 → played **raise 2×**; new set: check/call 0.77, raise ½ 0.21, raise pot 0.01
  - preflop board -              key `45` to call 1,500 → played **check/call**; new set: fold 0.91, check/call 0.09
