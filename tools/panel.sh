...
# SHAPES picks the shapes; the default is all nine (five calibrated to bots we
# have played, four to the published corpus).
...
row="| $LABEL |"
for kind in ${SHAPES:-station nit maniac foldraise hoops sticky nofold3bet folder wildpassive}; do
...
echo "| set | $(echo ${SHAPES:-station nit maniac foldraise hoops sticky nofold3bet folder wildpassive} | tr ' ' '|' | sed 's/|/ | /g') |"; echo "$row"
